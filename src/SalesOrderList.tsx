...
export  default class List extends React.Component<IProps,IState,string|number>{
    ...
    onClickHandel=(event: any )=>{
        this.props.onClick({index:Number(event.currentTarget.id),type:"ISalesOrder"})
    ...
    render(){
        ...
        return(<div>
            ...
            {Object.values(this.props.header).map((item:string,index)=>
                <TableCell key={index} align="left" style={{fontWeight: "bold"}}>{item as string}</TableCell>
            ...
            <TableBody>
                {this.props.data.map((item,index:number)=>
                    <TableRow key={item.MfgPartNo}  onMouseOver={this.handelMouseOver}  onMouseLeave={this.handelMouseLeave}>
