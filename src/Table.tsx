...
import {IHsales,IRmaRequestLineItems,ISalesOrder,IinvoicesList,InvoiceLine} from "./Interfaces"
let Sales_header={
...
export  default class Table extends React.Component<{},IState>{
    ...
    componentWillMount(){ //bring data here from API
    ...
    check=(obj:IRmaRequestLineItems)=>{
        return this.state.result.filter((obj2,indedx)=>obj.MnjPartNo==obj2.MnjPartNo).length==0
        
    }
    onClickhandel=(ev:{index:number,type:string})=>{  // Handel Clicks
            if(ev.type=="InvoiceLine"){ /// the type here is InvoiceLine
                let temp=info["Invoice Line Items"][ev.index];
               let temp2=info["Sales order Line  Items"].filter(obj=>obj.MfgPartNo==temp.MfgPartNo)
    ...
    render(){
        ...
        <div>
            ...
            <List onClick={this.onClickhandel} data={this.state.head2} header={Sales_header} />
            <br/>
            <h4 style={{margin:0}}>Invoice List</h4>
            <InvoicesList onClick={this.onClickhandel}  data={this.state.data2} header={InvoicesList_header} />
            <br/>
            <h4 style={{margin:0}}>Invoice Line Items</h4>

            <InvoiceLine_Items onClick={this.onClickhandel}  data={this.state.data3} header={InvoicesLine_header} />
           <br/>
